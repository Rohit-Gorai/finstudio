...
try { execSync("npm test", { stdio: "inherit" }); } catch { failures.push("Vitest failed; see test output above."); }
try { execSync("npm run build", { stdio: "inherit" }); } catch { failures.push("Production build failed; see build output above."); }
if (failures.length) { console.error("\nFINSTUDIO CURRICULUM AUDIT: FAIL"); failures.forEach(f => console.error(`- ${f}`)); process.exit(1); }
