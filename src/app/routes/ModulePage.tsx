...
import { curriculum, topicSlug } from "@/data/masterCurriculum";

export function ModulePage() {
  const { moduleId } = useParams();
  const level = curriculum.find((x) => String(x.level) === moduleId);
  ...
  return <div className="mx-auto max-w-6xl px-5 py-12 sm:px-8"><p className="font-mono text-xs font-semibold uppercase tracking-widest text-green">LEVEL {level.level} · {level.title}</p><h1 className="mt-3 font-display text-5xl font-semibold tracking-tight">{level.title}</h1><p className="mt-4 max-w-3xl text-lg text-ink-soft">{level.blurb}</p><div className="mt-10 grid gap-5 md:grid-cols-2">{level.modules.map((m)=><article key={m.title} className="rounded-xl border border-line bg-paper-2 p-6"><h2 className="font-display text-2xl font-semibold">{m.title}</h2><div className="mt-4 space-y-1">{m.topics.map((topic)=><Link key={topic} to={`/topic/${topicSlug(topic)}`} className="block rounded px-3 py-2 text-sm text-ink-soft no-underline hover:bg-paper-3 hover:text-green">{topic} <span aria-hidden="true">→</span></Link>)}</div></article>)}</div></div>;
}
