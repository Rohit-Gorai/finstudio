...
export function TopicPage() {
  ...
  return <div className="mx-auto max-w-5xl px-5 py-10 sm:px-8 sm:py-14">
    ...
    <section className="mt-5 rounded-xl border border-line bg-paper-2 p-6 sm:p-8"><p className="font-mono text-xs text-ink-faint">08 · MASTER</p><h2 className="mt-2 font-display text-3xl font-semibold">Teach it back</h2><p className="mt-3 text-ink-soft">Explain the concept, its formula or intuition, one real-world use, and one common mistake. Then move forward.</p>{next && <Link className="btn-primary mt-6 inline-flex px-4 py-2" to={`/topic/${topicRouteSlug(next)}`}>Next: {next.topic} →</Link>}</section>
  </div>;
}
