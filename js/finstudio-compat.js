...
(function () {
  ...
  function topicId(level, module, title) {
  ...
  function fallback(id, title, module, level) {
    ...
    if (/margin|ratio|return|multiple|yield|rate|turnover|coverage|moic|irr|beta|alpha|sharpe|peg/.test(lower)) {
    ...
    } else if (/value|valuation|price|revenue|profit|cash|debt|equity|cost|capital/.test(lower)) {
      formula = "Outcome = economic drivers × assumptions\nChange one driver at a time and explain why the result moves.";
    }
    return {
      id: id, minutes: 7, title: title, short: title,
      desc: "A practice-first lesson on " + title + " for Level " + level + ".",
      lede: "Understand " + title + " from first principles, see it in a realistic finance situation, then practise it.",
      body: [
        { t: "h2", text: "What is this?" },
        { t: "p", h: esc(title + " is a finance concept used to understand, measure or make decisions about a business, investment or transaction.") },
        { t: "h2", text: "Why does it matter?" },
        { t: "p", h: esc("You will encounter " + title + " in " + module + ". The goal is to understand what it measures, what drives it, where it appears and when it can mislead you.") },
        { t: "h2", text: "How it works" },
        { t: "p", h: "<strong>1.</strong> Define the concept.<br><br><strong>2.</strong> Identify its inputs and outputs.<br><br><strong>3.</strong> Apply the relevant relationship.<br><br><strong>4.</strong> Interpret the result in business context." },
        ...
        { t: "h2", text: "Sandbox" },
        { t: "sandbox", title: esc(title) + " — scenario sandbox", prompt: "Change the assumptions, predict the direction first, then explain the result.", kind: "opportunity", fields: [
        ...
        { t: "h2", text: "Apply it" },
        { t: "note", h: "<strong>Mini case.</strong> You are preparing an investment or transaction review and " + esc(title) + " has moved materially year over year. Trace the movement to its business drivers and assumptions. Then write: <em>what changed, why it changed, and whether you believe it is durable.</em>" },
        { t: "h2", text: "Common mistakes" },
        { t: "p", h: "<ul><li>Using the concept without checking its precise definition.</li><li>Ignoring inputs, denominators or timing.</li><li>Confusing a reported result with its economic explanation.</li><li>Comparing unlike businesses without understanding the differences.</li></ul>" },
        { t: "h2", text: "Master" },
        { t: "note", h: "<strong>Interview challenge:</strong> Explain " + esc(title) + " in 30 seconds, give a simple ₹ example, and name one way the concept could mislead you." },
        { t: "note", h: "<strong>Remember:</strong> Learn → See → Try → Practice → Build → Check → Apply → Master." }
      ]
    };
  }
  function makeAllTopicsRoutes() {
    ...
    LS.curriculumMap.forEach(function (lv) {
      lv.modules.forEach(function (mod) {
        mod.topics.forEach(function (topic) {
          var oldId = topic.id;
          var id = topicId(lv.level, mod.title, topic.title);
          if (seen[id]) id += "-" + (++seen[id]); else seen[id] = 1;
          var lesson = LS.lessons[oldId];
          if (!lesson) {
            Object.keys(LS.lessons).some(function (key) {
              var candidate = LS.lessons[key];
              if (candidate && candidate.title === topic.title) { lesson = candidate; return true; }
              return false;
            });
          }
          ...
          if (!LS.lessons[id]) {
            ...
            copy.id = id; copy.title = topic.title; copy.short = copy.short || topic.title;
            LS.lessons[id] = copy;
          }
          topic.id = id;
          topic.written = true;
    ...
    /* Preserve the previously broken public legacy URL. */
    ...
    LS.curriculumMap.some(function (lv) {
      return lv.modules.some(function (mod) {
        return mod.topics.some(function (topic) {
          if (lv.level === 9 && topic.title === "Convexity") { convexityId = topic.id; return true; }
          return false;
        });
      });
    });
  ...
  function normalizeLegacyPath() {
    ...
    if (location.hash !== next) { location.hash = next; return true; }
    return false;
  }
  function enhanceCurriculum() {
    var page = document.querySelector(".roadmap-page");
    if (!page) return;
    Array.prototype.forEach.call(page.querySelectorAll(".roadmap-level"), function (section, i) { section.id = "level-" + i; });
    Array.prototype.forEach.call(page.querySelectorAll(".rt-planned"), function (node) {
      var topic = node.textContent.trim();
      if (!topic) return;
      var a = document.createElement("a"); a.className = "rt rt-universal"; a.href = "#/topic/" + slug(topic); a.textContent = topic; a.title = "Open interactive FinStudio lesson"; node.replaceWith(a);
    });
  }
  function scrollLevel() {
    var match = location.hash.match(/^#\/?curriculum#(level-\d+)$/);
    if (!match) return;
    setTimeout(function () { var target = document.getElementById(match[1]); if (target) target.scrollIntoView({ behavior: "smooth", block: "start" }); }, 100);
  }
  function sync() {
