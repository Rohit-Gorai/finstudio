...
export function CurriculumPage() {
  const lessonByKey = new Map(allLessons.map(l => [`${l.level}|${l.module}|${l.title}`, l]));
  ...
  return <div className="mx-auto max-w-6xl px-5 py-12 sm:px-8">
    <header className="max-w-4xl">
      ...
      <div className="mt-5 flex flex-wrap gap-3 text-sm">
        ...
        <span className="rounded-full border border-green bg-green-soft px-3 py-1 font-semibold text-green">{stats.authored} full lessons written</span>
        <span className="rounded-full border border-line bg-paper-2 px-3 py-1">{stats.draft} drafts · {stats.outline} not yet written</span>
        <span className="rounded-full border border-line bg-paper-2 px-3 py-1">{overall.done}/{overall.total} marked complete by you</span>
      </div>
    </header>
        ...
        <div className="mt-5 grid gap-4 md:grid-cols-2">
          {level.modules.map(m=>{ const mp=moduleProgress(level.level,m.title); return <article key={m.title} className="rounded-lg border border-line bg-paper-2 p-5 shadow-sm">
            ...
            <div className="mt-4 grid gap-2 sm:grid-cols-2">
              {m.topics.map((topic,topicIndex)=>{ const lesson=lessonByKey.get(`${level.level}|${m.title}|${topic}`) ?? findLesson(topic); if(!lesson) return null; const label = lesson.status==="authored" ? "Full lesson" : lesson.status==="draft" ? "Draft" : "Not yet written"; return <Link key={topic} to={`/lesson/${lesson.id}`} aria-label={`Open lesson: ${topic}`} className="group rounded-lg border border-line bg-paper px-3 py-2.5 text-sm text-ink-soft no-underline transition hover:-translate-y-px hover:border-green hover:bg-green-soft hover:text-green focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-green focus-visible:ring-offset-2">
                <span className="flex items-center justify-between gap-2"><span><span className="mr-2 font-mono text-[10px] text-ink-faint">{String(topicIndex+1).padStart(2,"0")}</span>{topic}</span><span aria-hidden="true" className="font-semibold">→</span></span><span className="mt-1 flex gap-1.5 text-[10px] font-semibold uppercase tracking-wider text-ink-faint"><span>{label}</span></span>
              </Link>;})}
            </div>
          </article>;})}
        </div>
